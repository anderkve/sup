...
A sup run mode that produces a 1d graph of the delta chi-square,
chi^2(x) - chi^2_min, for variable x, showing the minimum delta chi-square in each bin.
...
def run(args):
    """The main function for the 'chisq1d' run mode.
    ...
    if not y_range:
        y_range = [0.0, np.max(y_data)] # Max will be updated after y_data is calculated if not user-specified
    ...
    x_label = x_name
    y_label = "delta chi-square"
    s_label = "delta chi-square"
